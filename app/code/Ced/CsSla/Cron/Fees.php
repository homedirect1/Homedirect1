...
namespace Ced\CsSla\Cron;

/**
 * Class Fees
 * @package Ced\CsSla\Cron
 */
class Fees
{
    /**
                ...
                if ($vorder['order_status'] == \Ced\CsSla\Helper\Data::STATE_CONFIRMED) {
                    ...
                    foreach ($orderItems as $item) {
                        ...
                        if ($flag && !isset($orderFeeId)) {
                            ...
                            $description = 'your order with orderId ' . $vorder['order_id'] . ' is delayed for dispatch please dispatch it as soon as possible';
                            ...
                            $OFees = $this->orderfeeFactory->create()->getCollection()->addFieldToFilter('status', 0)->addFieldToFilter('order_id', $incrementId)->addFieldToFilter('vendor_id', $vendorId)->addFieldToFilter('fee_id', ['nin' => 'sla_dispatch_fee'])->getData();
    ...
    public function autoCancel()
    {
        ...
        $autocancel = $this->slaHelper->getStoreConfig('ced_csmarketplace/vsla/auto_cancel');

        $date = $this->dateTime->gmtDate();
        $days_ago = date('Y-m-d h:i:s', strtotime('-' . $autocancel . ' days', strtotime($date)));

        $vorders = $this->vordersFactory->create()->getCollection()->addFieldToFilter('order_status', ['null' => true])->addFieldToFilter('created_at', ['lt' => $days_ago]);
        foreach ($vorders as $vorder) {
            if (!$vorder->canInvoice() && !$vorder->canShip()) {
                ...
                if ($isVendorOrder) {
                    ...
                    $email = $this->orderFactory->create()->load($vorder['order_id'], 'increment_id')->getCustomerEmail();
                    $description = 'your order with orderId ' . $vorder['order_id'] . ' is cancelled due to delay in confirmation';
            ...
            if ($vorder->canShip()) {
                if ($vorder->canCancel()) {
                    ...
                    $vOrderCollection = $this->vordersFactory->create()
                        ->getCollection()->addFieldToFilter('order_id', $incrementId)->getData();
                    ...
                    if ($isVendorOrder) {
                        ...
                        $email = $this->orderFactory->create()->load($vorder['order_id'], 'increment_id')->getCustomerEmail();
                        $description = 'your order with orderId ' . $vorder['order_id'] . ' is cancelled due to delay in Dispatch';
