            ...
            if (!empty($productTypes) || !empty($categoryWise)) {
                ...
                foreach ($order->getAllItems() as $item) {
                    ...
                    if ($item->getVendorId() && $item->getVendorId() == $vendorId) {
                        ...
                        if($isCategoryExist){
                            $cw = '';
                            if(isset($category_temp_priority['vendor']) && $category_temp_priority['vendor'] == $vendorId){
                                $cw = isset($category_temp_priority['fee']) ? $category_temp_priority['fee'] : 0;
                            }elseif($category_temp_priority['vendor'] === 0 || $category_temp_priority['vendor'] == '0'){
                                $cw = isset($category_temp_priority['fee']) ? $category_temp_priority['fee'] : 0;
                            }
                            ...
                            if($salesCalMethod == 'percentage'){
                                $cw =  $this->helper->calculateCommissionFee(
