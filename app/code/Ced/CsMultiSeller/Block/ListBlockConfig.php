 ...
 */
class ListBlockConfig extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    public function _construct()
    {
        ...
        if ($this->_coreRegistry->registry('product') != null) {
            $parentId = $this->_coreRegistry->registry('product')->getEntityId();
            if ($parentId > 0) {
        ...
        $minPrice = 0;
        if ($this->marketplaceHelper->getStoreConfig(
            'ced_csmarketplace/ced_csmultiseller/minprice',
    ...
    public function ConfigurableChild($product_id){
        $product_collection = [];
        $products = [];
        $parentId = $this->_coreRegistry->registry('product')->getEntityId();
        if ($parentId > 0) {
