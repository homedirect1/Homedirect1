...
class ConfigurablePanel extends AbstractModifier {
	 ...
	 */
	public function __construct(LocatorInterface $locator, UrlInterface $urlBuilder, $formName, $dataScopeName, $dataSourceName, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig,\Magento\Framework\App\Request\Http $request,$associatedListingPrefix = '') {
						...
						'arguments' => [
								'data' => [
										'config' => [
												...
												'sortOrder' => $this->getNextGroupSortOrder ( $meta, self::$groupContent, self::$sortOrder )
										]
								]
						],
												...
												'options' => [
														...
														'buttons' => [
																[
																		...
																		'actions' => [
																				[
																						'targetName' => 'ns= ' . $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING . ', index=' . static::ASSOCIATED_PRODUCT_LISTING,
																						'actionName' => 'save'
																				],
																				'closeModal'
																		]
										...
										'arguments' => [
												'data' => [
														'config' => [
																...
																'content' => __ ( 'Choose a new product to delete and replace' . ' the current product configuration.' ),
																'imports' => [
																		'visible' => '!ns = ${ $.ns }, index = ' . ConfigurablePanel::CONFIGURABLE_MATRIX . ':isEmpty'
																]
														]
												]
										]
										...
										'arguments' => [
												'data' => [
														'config' => [
																...
																'component' => 'Magento_Ui/js/form/components/html',
																'additionalClasses' => 'message message-notice',
																'content' => __ ( 'For better results, add attributes and attribute values to your products.' ),
																'imports' => [
																		'visible' => 'ns = ${ $.ns }, index = ' . ConfigurablePanel::CONFIGURABLE_MATRIX . ':isEmpty'
																]
														]
										...
										'arguments' => [
												'data' => [
														'config' => [
																...
																'dataScope' => $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING,
																'externalProvider' => $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING . '.data_source',
																'selectionsProvider' => $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING . '.' . $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING . '.product_columns.ids',
																...
																'currentProductId' => $model->getId (),
																'dataLinks' => [
																		'imports' => false,
																		'exports' => true
																],
																'changeProductProvider' => 'change_product',
																'productsProvider' => $this->associatedListingPrefix . 'configurable_associated_product_listing.data_source',
																'productsColumns' => $this->associatedListingPrefix . 'configurable_associated_product_listing' . '.' . $this->associatedListingPrefix . 'configurable_associated_product_listing.product_columns',
																'productsMassAction' => $this->associatedListingPrefix . 'configurable_associated_product_listing' . '.' . $this->associatedListingPrefix . 'configurable_associated_product_listing.product_columns.ids',
																'modalWithGrid' => 'ns=' . $this->formName . ', index=' . static::ASSOCIATED_PRODUCT_MODAL,
																'productsFilters' => $this->associatedListingPrefix . 'configurable_associated_product_listing' . '.' . $this->associatedListingPrefix . 'configurable_associated_product_listing.listing_top.listing_filters'
														]
												]
										]
								]
				...
				'arguments' => [
						'data' => [
								'config' => [
										...
										'label' => false,
										'content1' => __ ( 'Configurable products allow customers to choose options ' . '(Ex: shirt color). You need to create a simple product for each ' . 'configuration (Ex: a product for each color).' ),
										'content2' => __ ( 'Configurations cannot be created for a standard product with downloadable files. ' . 'To create configurations, first remove all downloadable files.' ),
								...
								'arguments' => [
										'data' => [
												'config' => [
														...
														'actions' => [
																[
																		'targetName' => 'ns=' . $this->formName . ', index=' . static::ASSOCIATED_PRODUCT_MODAL,
														...
														'sortOrder' => 10,
														'imports' => [
																'visible' => 'ns = ${ $.ns }, index = ' . ConfigurablePanel::CONFIGURABLE_MATRIX . ':isShowAddProductButton'
														]
												]
										]
								]
				...
				'arguments' => [
						'data' => [
								'config' => [
										...
										'modalWithGrid' => 'ns=' . $this->formName . ', index=' . static::ASSOCIATED_PRODUCT_MODAL,
										'gridWithProducts' => 'ns=' . $this->associatedListingPrefix . static::ASSOCIATED_PRODUCT_LISTING . ', index=' . static::ASSOCIATED_PRODUCT_LISTING
								]
						]
				],
	...
	protected function getRows() {
		return [
				'record' => [
						...
						'children' => [
								'thumbnail_image_container' => $this->getColumn ( 'thumbnail_image', __ ( 'Image' ), [
										'fit' => true,
										...
										'uploaderConfig' => [
												'url' => $this->urlBuilder->getUrl ( 'csproduct/product_gallery/upload' )
										],
										'dataScope' => 'image'
								], [
								...
								'name_container' => $this->getColumn ( 'name', __ ( 'Name' ), [ ], [
								...
								'price_container' => $this->getColumn ( 'price', __ ( 'Price' ), [
										'imports' => [
								...
								'quantity_container' => $this->getColumn ( 'quantity', __ ( 'Quantity' ), [
										'dataScope' => 'qty'
								], [
										'dataScope' => 'qty'
								] ),
	 ...
	 * @return array
	 */
	protected function getColumn($name, \Magento\Framework\Phrase $label, $editConfig = [], $textConfig = []) {
		...
		$fieldEdit ['arguments'] ['data'] ['config'] = array_replace_recursive ( $fieldEdit ['arguments'] ['data'] ['config'], $editConfig );
		$fieldText ['arguments'] ['data'] ['config'] = array_replace_recursive ( $fieldText ['arguments'] ['data'] ['config'], $textConfig );
