...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
        ...
        foreach ($return['points_data'] as $pkey => $point_data) {
            ...
            $new_expiration_point_collection = array_filter(
                ...
                function ($values) use ($create_datetime, $id) {
                    return (isset($values['expiration_date']) && $values['status'] == \Magento\Sales\Model\Order::STATE_COMPLETE
                        && (
                            (
            ...
            */
            ...
            foreach ($expiration_point_collection as $key => $data) {
                ...
                } elseif (strtotime($data['expiration_date']) > $current_date) {
                    $non_expired_points += $data['point'];
                }
     ...
     * @return bool|int
     */
    public function sendInvitationEmail($emails, $subject, $referral_url, $customer_Id)
    {
        try {
            ...
            $template = $this->scopeConfig->getValue(
                'reward/referral/referal_email_template',
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            );
            $this->_inlineTranslation->suspend();
            $senderInfo = [
                'name' => 'SUPPORT',
                'email' => 'info@homedirect.in',
//                'email' => $support,
            ];
            if (!empty($support)) {
                $sent = 0;
