        ...
        foreach ($dataSource['data']['items'] as $key => $item) {
            ...
            foreach ($amountDesc as $incrementId => $baseNetAmount) {
                if(is_array($baseNetAmount)){
                    $html .= '<label for="order_id_' . $baseNetAmount['order_id'] . '"><b>'. __('Order') .'# </b>' . $baseNetAmount['order_id'].
                        '</label>, <br> <b>'. __('Amount') . ' </b>' . $currency . number_format($baseNetAmount['order_total'],2) . '<br/>';
