 ...
 */
class Vendorpayment extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer
{
    /**
    ...
    public function render(\Magento\Framework\DataObject $order)
    {
        ...
        if ($order->getPaymentState() == 1 || $order->getPaymentState() == 3 || $order->getPaymentState() == 4) {
            if ($this->advHelper->getOrderPaymentType($order->getOrderId()) == __('PrePaid')) {
                ...
                $vendorPay = $this->advHelper->vendorPay($order->getVendorId(), $order->getId());
                $payment = (float)$vendorPay;
                echo $this->priceCurrency->format($payment, false, 2, null, $currencyCode);
            }
        } else {
            echo $this->priceCurrency->format($order->getVendorEarn(), false, 2, null, $currencyCode);
        }
    }
}
