	...
	public function execute() {
		...
		if ($data) {
			...
			if ($id) {
				...
				foreach($allRules->getData() as $key => $value) {
					if($value['discount_amount'] == $data['discount_amount'] 
						&& $value['rule_name'] != $data['rule_name']
					) {
			    ...
			    $allRules = $this->denominationCollection
				    ...
				    ->getFirstItem();
				$allRulesName = $this->denominationRules->getCollection()->addFieldToSelect('*')
				->addFieldToFilter('rule_name', $data['rule_name']);
				...
				} elseif(sizeof($allRulesName->getData())>0) {
					$this->messageManager->addErrorMessage (__('Rule already exists for Rule name %1',
						$data ['rule_name']));
						return $resultRedirect->setPath ( '*/discount/edit', [ 
							'id' => $this->getRequest ()->getParam ( 'id' ) 
						] );
		    ...
		    if($data['discount_amount'] > 0 && $data['cart_amount'] > 0) {
				...
				$this->messageManager->addErrorMessage( __ ( 'Discount amount & Cart Amount must be greater than 0' ) );
				return $resultRedirect->setPath ( '*/discount/edit', [ 
					'id' => $this->getRequest ()->getParam ( 'id' ) 
				] );
			}
