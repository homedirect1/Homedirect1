...
namespace Ced\CsHyperlocal\Plugin\Adminhtml\Vendor;
...
class Save
{
     ...
     * @return mixed
     */
    public function aroundExecute(\Ced\CsMarketplace\Controller\Adminhtml\Vendor\Save $subject, \Closure $proceed)
    {
        $returnValue = $proceed();
        $city = false;
        $state = false;
       ...
       // $data['latitude'] = $lat;
        ...
        $url = 'https://maps.googleapis.com/maps/api/geocode/json?&sensor=false&key=' . $apiKey;
        $ch = curl_init();
        ...
        /** save zipcode*/
        $zipcodeCollection = $this->zipcodeCollection->create()->addFieldToFilter('location_id', $locationId);
        if (isset($data['zipcode']) && $data['zipcode'] != '') {
            if ($zipcodeCollection->count()) {
                $this->zipcodeFactory->create()
                    ->load($zipcodeCollection->getFirstItem()->getId())
